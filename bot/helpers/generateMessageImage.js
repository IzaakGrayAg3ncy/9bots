...
async function createDiscordMessageImage(quote, nickname, avatarURL, outputPath = './output.png', isGolden = false) {
    const width = 800;
    const avatarSize = 50;
    const padding = 20;
    const fontSize = 16;
    const usernameFontSize = 20;
    const lineHeight = 1.375 * usernameFontSize;
    const timestampFontSize = 12;
    const backgroundColor = isGolden ? '#FFFFFF' : '#313338'; // White for golden
    const usernameColor = isGolden ? '#B8860B' : '#ad2440'; // Darker gold for golden
    const textColor = isGolden ? '#B8860B' : '#DCDDDE'; // Darker gold for golden
    ...
    try {
        ...
        console.error('Error fetching or processing avatar:', error);
    }
}
